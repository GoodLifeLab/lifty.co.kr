...
import { generateEmailVerificationCode, saveEmailVerificationCode, sendVerificationEmail } from '@/utils/email';

export async function POST(req: NextRequest) {
  try {
    const { email } = await req.json();
    if (!email) {
      return NextResponse.json({ success: false, error: '이메일은 필수입니다.' }, { status: 400 });
    }
    ...
    return NextResponse.json({ success: false, error: error.message || '이메일 전송 실패' }, { status: 500 });
  }
} 